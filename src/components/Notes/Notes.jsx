...
const Notes = ({ uid, compByDetailPage }) => {
  // Contexts
  const {
    selectedText,
    setSelectedText,
  ...
  return (
    <div>
      ...
      <div>
        ...
        <div className="px-3 py-1 flex justify-between items-center text-[#14395d] ">
          ...
          <AiOutlineCloseCircle
            onClick={() => setMasterNotesFlag(false)}
        ...
        <div className={`${(detailedNotes || noteEditID) && "hidden"}`}>
          <div className="flex flex-col justify-between items-center px-3 py-2 gap-1">
            {/* Filter Options */}
            <div
              className={`flex gap-2 w-full items-center bg-white flex-wrap`}
            >
              {/* Companies*/}
              <Select
                ref={selectCompRef}
                ...
                onChange={(values) => {
                  values
                    ? (setCompFilter(values?.FINCODE),
                      setSelectedComp(values?.Company_Name),
                      setDefaultCompanyForDetailedNotes({
      ...
      {!detailedNotes && !noteEditID ? (
        ...
        <DetailedNotes uid={uid} fincode={compFilter} compName={selectedComp} />
      ) : (
        noteEditID && <NoteEdit />
      )}
      {/* .....MAIN End..... */}
      ...
      >
        ...
        <div className="bg-white p-1 rounded-md">
          <Select
            ...
            //   }),
            // }}
            className="text-[#0d2843] font-bold text-sm flex-1 "
            options={noteMaster?.comp_dropdown}
            getOptionLabel={(option) => option.Company_Name}
            getOptionValue={(option) => option.FINCODE}
