...
const IndicesStatus = () => {
  ...
  async function postNewNoteFunc() {
    const res = await fetch(
      `https://transcriptanalyser.com/market/indices_status`,
      {
        method: "POST",
        headers: {
  ...
  return (
    <div className="p-2 bg-white border-[#4577A8] border-[1px] rounded-md shadow-md">
      ...
      {/* Heat Maps */}
      <div className="flex gap-3 flex-wrap justify-center md:max-h-[200px] overflow-y-auto scrollbar-thin px-2">
        {data ? (
          data?.data?.map((item) => (
            <div
              onClick={() => (
              ...
              key={item?.Index_Code}
              className={`flex items-center text-sm justify-center text-white text-center rounded-md font-semibold min-w-[90px] max-w-[90px] h-[60px] cursor-pointer hover:underline flex-1 relative ${
